...
async def configure_event_handlers(client, user_id):
    ...
    @client.on(events.NewMessage(pattern=r'^01ping$'))
    async def ping_handler(event):
        """Tes koneksi bot."""
        await event.reply("\U0001F3D3 Pong! Bot aktif.")
        message_count[get_today_date()] += 1

    @client.on(events.NewMessage(pattern=r'^01bcstargr (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^01bcstargr(\d+) (\d+[smhd]) (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^01stopbcstargr(\d+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^01setreply (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^01help$'))
    async def help_handler(event):
        """Tampilkan daftar perintah."""
        help_text = (
            "\U0001F4AC Daftar Perintah:\n"
            "01ping - Cek status bot\n"
            "01bcstargr <pesan> - Kirim broadcast ke semua chat\n"
            "01bcstargr<nomor> <interval> <pesan> - Kirim broadcast ke grup tertentu\n"
            "01stopbcstargr<nomor> - Hentikan broadcast ke grup tertentu\n"
            "01setreply <pesan> - Set auto-reply untuk chat masuk\n"
            "01help - Menampilkan daftar perintah"
        )
